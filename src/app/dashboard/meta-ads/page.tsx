            ...
            <tbody>
              {result.data.length === 0 ? (
                ...
                result.data.map((ad) => (
                  ...
                  >
                    <td className="px-4 py-3 text-gray-900">{formatDate(ad.report_date)}</td>
                    <td className="px-4 py-3 text-gray-900">{ad.campaign_name ?? "-"}</td>
                    <td className="px-4 py-3 text-gray-500 text-right">{ad.spend != null ? formatCurrency(ad.spend) : "-"}</td>
                    <td className="px-4 py-3 text-gray-500 text-right">{ad.impressions != null ? formatNumber(ad.impressions) : "-"}</td>
                    <td className="px-4 py-3 text-gray-500 text-right">{ad.clicks != null ? formatNumber(ad.clicks) : "-"}</td>
                    <td className="px-4 py-3 text-gray-500 text-right">{ad.leads != null ? formatNumber(ad.leads) : "-"}</td>
                    <td className="px-4 py-3 text-gray-500 text-right">{ad.ctr != null ? formatPercent(ad.ctr) : "-"}</td>
                    <td className="px-4 py-3 text-gray-500 text-right">{ad.cpc != null ? formatCurrency(ad.cpc) : "-"}</td>
                    <td className="px-4 py-3 text-gray-500 text-right">{ad.cpl != null ? formatCurrency(ad.cpl) : "-"}</td>
                  </tr>
                ))
              )}
            </tbody>
